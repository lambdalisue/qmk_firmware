...
[_JS] = LAYOUT_ergodox(
      ...
      KC_NUHS, KC_N,    KC_M,    KC_COMM, KC_DOT,  KC_SLSH, K_UANDS,
                        K_1ANDS, KC_LEFT, KC_DOWN, KC_UP,   KC_RGHT,
      KC_INS,  KC_SLCK,
      KC_BRK,
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch (keycode) {
        ...
        case KC_LBRC:
        case KC_GRV:
            if ((keycode == KC_LBRC && !is_jis_layer) || (keycode == KC_GRV && is_jis_layer)) {
                return true;
            }
        ...
        case K_2ANDC:
            if (!record->event.pressed && pseudo_escape_pressed) {
                ...
                send_keyboard_report();
                add_key(is_jis_layer ? KC_LBRC : KC_GRV);
                send_keyboard_report();
                return false;
            }
            if (!record->event.pressed && pseudo_backspace_pressed) {
